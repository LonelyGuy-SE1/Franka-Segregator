...
import xacro
import xacro.substitution_args as _sa
try:
    ...
    try:
        ...
        if 'ROS_PACKAGE_PATH' not in os.environ:
            os.environ['ROS_PACKAGE_PATH'] = os.path.dirname(os.getcwd())
        _rospack = rospkg.RosPack()
        _sa._eval_find = lambda pkg: _rospack.get_path(pkg).replace(os.sep, '/')
    except ImportError:
        print('Error: Neither ament_index_python nor rospkg found.')
        print('Install one of them: pip install rospkg')
        exit(1)
